...
def test_wait_on_available_threads(settings):
    ...
    assert any(name.endswith(wait_on_available_threads.__name__) for name in settings.global_callback_strings)
...
def test_running_enough_scripts_to_wait(settings):
    ...
    count = settings.thread_count // THREADS_PER_RUN + 2  # at least 2 runs should be blocked
    ...
    ok, errors = wait_on_ok_errors(*runs, timeout=THREAD_POOL_FULL_WAIT_TIME_SECONDS * 2)
