...
|---|---|---|---|---|
| shell_input | `str` | `PydanticUndefined` | 0.2.0 | - |
| env | `dict[str, str]` | `...` | 0.2.0 | - |
| extra_popen_kwargs | `dict` | `...` | 0.2.0 | - |
| allow_non_zero_exit | `bool` | `False` | 0.2.0 | - |
| skip_os_env | `bool` | `False` | 0.2.0 | - |
| skip_binary_check | `bool` | `False` | 0.2.0 | - |
| skip_interactive_check | `bool` | `False` | 0.2.0 | - |
...
| run_output_dir | `Path | None` | - | 0.2.0 | Directory to store run logs, defaults to settings.run_logs /{XX}_{self.exec_name} |
| run_log_stem_prefix | `str` | `''` | 0.2.0 | Prefix for run log stem |
| skip_html_log_files | `bool` | `False` | 0.2.0 | Skip HTML log files, by default dumps HTML logs to support viewing colored output in browsers |
| skip_progress_output | `bool` | `False` | 0.2.0 | Skip transitive std out/err output, useful for large outputs that are not needed in the logs when running parallel scripts |
| terminal_width | `int | None` | `999` | 0.2.0 | - |
| is_binary_call | `bool` | - | 0.2.0 | Inferred if not provided |
| settings | `AskShellSettings` | `...` | 0.2.0 | - |
| message_callbacks | `list[Callable[[typing.Union[ask_shell._internal.events.ShellRunBefore, ask_shell._internal.events.ShellRunPOpenStarted, ask_shell._internal.events.ShellRunStdStarted, ask_shell._internal.events.ShellRunStdReadError, ask_shell._internal.events.ShellRunStdOutput, ask_shell._internal.events.ShellRunRetryAttempt, ask_shell._internal.events.ShellRunAfter]], bool | None]]` | `...` | 0.2.0 | Callbacks for run messages, useful for custom handling of stdout/stderr |
...
|---|---|---|---|
| config | `ShellConfig` | - | 0.2.0 |
| p_open | `Popen | None` | `None` | 0.2.0 |
| current_attempt | `int` | `1` | 0.2.0 |
...
| Field | Type | Default | Since |
|---|---|---|---|
| interrupt_message | `str` | - | 0.2.0 |
| immediate_kill | `bool` | `False` | 0.2.0 |
...
| task_name | `str` | - | 0.2.0 |
| total | `int` | `0` | 0.2.0 |
| max_concurrent_submits | `int` | `4` | 0.2.0 |
| threads_used_per_submit | `int` | `5` | 0.2.0 |
| sleep_time | `float` | `1` | 0.2.0 |
| sleep_callback | `Callable[[], Any] | None` | `None` | 0.2.0 |
| exit_wait_timeout | `float | None` | `None` | 0.2.0 |
| pool | `ThreadPoolExecutor` | `...` | 0.2.0 |
